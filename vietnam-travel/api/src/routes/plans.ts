...
import {
    getVivu360DestinationPlan,
    VIVU360_DESTINATION_PLAN_SLUG,
} from '../services/vivu360DestinationService.js';
...
router.get('/', async (_req, res, next) => {
    try {
        ...
        const vivu360Plan = await getVivu360DestinationPlan();
        res.json(vivu360Plan ? [{
            id: vivu360Plan.id,
            slug: vivu360Plan.slug,
            name: vivu360Plan.name,
            dateRange: vivu360Plan.dateRange,
            budgetLimit: vivu360Plan.budgetLimit,
        }] : []);
    } catch (error) {
        next(error);
    }
...
router.get('/:slug', async (req, res, next) => {
    try {
        ...
        if (req.params.slug === VIVU360_DESTINATION_PLAN_SLUG) {
            const vivu360Plan = await getVivu360DestinationPlan();
            if (vivu360Plan) {
                res.json(vivu360Plan);
                return;
