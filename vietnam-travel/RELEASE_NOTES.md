...
## Unreleased - Vivu360 Mobile Map Integration

- Connected the new travel map to the Vivu360 mobile app through WebView.
- Added a fallback travel plan backed by destination data from Vivu360 API
  when the local SQLite database has no configured plans.
- Standardized the local Travel API port on `7321` and built the public map
  under `/travel-map/` for serving from the main Vivu360 API.
